...
try:
    import resource
except ImportError:  # Windows has no resource module; peak-RSS is unavailable
    resource = None
...
def run_benchmarks():
    ...
    for n in [1_000, 10_000, 50_000]:
        ...
        for i, v in enumerate(vecs):
            mem.remember(
                f"memory {i}: content about topic {i % 100} with details {hashlib.md5(str(i).encode()).hexdigest()}",  # noqa: E501 - benchmark print/format string
                embedding=v,
            )
        elapsed = time.perf_counter() - t0
        per_op = elapsed * 1000 / n
        throughput = n / elapsed
        print(f"  {n:>6,} memories: {per_op:.3f} ms/op  |  {throughput:,.0f} inserts/s  |  total: {elapsed:.2f}s")  # noqa: E501 - benchmark print/format string
        RESULTS[f"insert_{n}"] = {"per_op_ms": round(per_op, 3), "throughput": round(throughput, 0), "total_s": round(elapsed, 2)}  # noqa: E501 - benchmark print/format string
        mem.close()
    ...
    for n in [1_000, 10_000, 50_000]:
        ...
        RESULTS[f"vector_search_{n}"] = {"avg_ms": round(float(avg), 3), "p50_ms": round(float(p50), 3), "p99_ms": round(float(p99), 3)}  # noqa: E501 - benchmark print/format string
        mem.close()
    ...
    for n in [1_000, 10_000, 50_000]:
        ...
        RESULTS[f"fts_search_{n}"] = {"avg_ms": round(float(avg), 3), "p50_ms": round(float(p50), 3), "p99_ms": round(float(p99), 3)}  # noqa: E501 - benchmark print/format string
        mem.close()
    ...
    for n in [1_000, 10_000, 50_000]:
        ...
        vecs = np.random.randn(n, DIM).astype("float32")
        for i, v in enumerate(vecs):
            mem.remember(f"memory {i}: deploy to production using kubernetes cluster {i % 50}", embedding=v)  # noqa: E501 - benchmark print/format string
        ...
        p99 = np.percentile(times, 99)
        print(f"  {n:>6,} memories: avg={avg:.3f}ms  p50={p50:.3f}ms  p99={p99:.3f}ms")
        RESULTS[f"hybrid_search_{n}"] = {"avg_ms": round(float(avg), 3), "p50_ms": round(float(p50), 3), "p99_ms": round(float(p99), 3)}  # noqa: E501 - benchmark print/format string
        mem.close()
    ...
    for n in [1_000, 10_000, 50_000]:
        ...
        p99 = np.percentile(times, 99)
        print(f"  {n:>6,} memories: avg={avg:.3f}ms  p50={p50:.3f}ms  p99={p99:.3f}ms")
        RESULTS[f"recall_{n}"] = {"avg_ms": round(float(avg), 3), "p50_ms": round(float(p50), 3), "p99_ms": round(float(p99), 3)}  # noqa: E501 - benchmark print/format string
        mem.close()
    ...
    n_connected = len(last_result.get("entities", [])) if isinstance(last_result, dict) else len(last_result)  # noqa: E501 - benchmark print/format string
    ...
    near_dupes = [
        ("The quick brown fox jumps over the lazy dog in the park", "A quick brown fox leaps over the lazy dog in the park"),  # noqa: E501 - benchmark print/format string
        ("Deploy to production using kubernetes with rolling updates", "Deploy to production using kubernetes via rolling updates"),  # noqa: E501 - benchmark print/format string
        ("Machine learning model training requires GPU acceleration", "Machine learning model training needs GPU acceleration"),  # noqa: E501 - benchmark print/format string
        ("The database connection pool has been exhausted completely", "The database connection pool is completely exhausted"),  # noqa: E501 - benchmark print/format string
        ("Python 3.12 includes significant performance improvements", "Python 3.12 brings significant performance improvements"),  # noqa: E501 - benchmark print/format string
    ]
    ...
    dupe_times = []
    detected = 0
    for _orig, near in near_dupes:
        t0 = time.perf_counter()
        is_dup, _score = dedup.is_duplicate(near)
    ...
    non_dup_detected = 0
    for nd in non_dupes:
        is_dup, _score = dedup.is_duplicate(nd)
        if is_dup:
            non_dup_detected += 1
    ...
    print(f"  True positive rate: {detected}/{len(near_dupes)} ({detected/len(near_dupes)*100:.0f}%)")  # noqa: E501 - benchmark print/format string
    print(f"  False positive rate: {non_dup_detected}/{len(non_dupes)} ({non_dup_detected/len(non_dupes)*100:.0f}%)")  # noqa: E501 - benchmark print/format string
    RESULTS["dedup"] = {
    ...
    for n in [1_000, 10_000]:
        ...
        vecs = np.random.randn(n, DIM).astype("float32")
        for i, v in enumerate(vecs):
            mem.remember(f"memory {i}: footprint test with realistic content length for a real agent memory", embedding=v)  # noqa: E501 - benchmark print/format string
        ...
        RESULTS[f"footprint_{n}"] = {"db_mb": round(db_size, 1), "per_memory_kb": round(per_mem_kb, 1)}  # noqa: E501 - benchmark print/format string
        mem.close()
    ...
    for n in [1_000, 10_000, 50_000]:
        ...
        RESULTS[f"cold_start_{n}"] = {"avg_ms": round(float(avg), 1), "p99_ms": round(float(p99), 1)}  # noqa: E501 - benchmark print/format string
